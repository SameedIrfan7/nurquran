...
body{background:var(--bg);color:var(--cream);font-family:'DM Sans',sans-serif;min-height:100vh;overflow-x:hidden;max-width:100vw}
body.light{
...
function SetupScreen({onDone}){
  ...
  const features=[
    {icon:'📖',ar:'اقْرَأْ',title:'Read the complete Quran',desc:'All 114 Surahs in continuous Mushaf style. Arabic + translations in 8 languages.'},
    {icon:'🎙',ar:'تَرْتِيلًا',title:'Daily recitations until Ramadan',desc:'One anonymous voice reciting every day from March 25, 2026 until Ramadan 2027.'},
    {icon:'🎵',ar:'صَوْت',title:'5 world-renowned Qaris',desc:'Alafasy, As-Sudais, Al-Husary, Al-Minshawi, Basfar. Tap any ayah to listen.'},
    {icon:'🏆',ar:'مُسَابَقَة',title:'Earn points & rank up',desc:'1 point per ayah. 50 bonus per surah. Compete on the global leaderboard.'},
    {icon:'🎓',ar:'عِلْم',title:'Learn from great scholars',desc:'Dr. Israr Ahmed, Mufti Menk, Nouman Ali Khan, Omar Suleiman and more.'},
  ...
  if(isMobile) return(
    <div style={{minHeight:'100vh',background:'var(--bg)',position:'relative',overflow:'hidden',display:'flex',flexDirection:'column'}}>
      <div style={{position:'absolute',top:0,left:0,right:0,height:'50vh',background:'radial-gradient(ellipse at 50% -10%,rgba(200,164,90,0.1) 0%,transparent 65%)',pointerEvents:'none'}}/>
      <div style={{flex:1,padding:'40px 28px 40px',display:'flex',flexDirection:'column',justifyContent:'center',position:'relative'}}>
        {/* Logo */}
        <div style={{marginBottom:28,textAlign:'center'}}>
          <div style={{fontFamily:'Amiri,serif',fontSize:11,color:'var(--gold)',opacity:0.5,letterSpacing:3,marginBottom:4}}>نور القرآن</div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:42,fontWeight:300,color:'var(--cream)',letterSpacing:3,lineHeight:1}}>NurQuran</div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14,fontStyle:'italic',color:'var(--cream3)',marginTop:4}}>The Light of the Quran</div>
        </div>
        {/* Bismillah */}
        <div style={{textAlign:'center',marginBottom:24}}>
          <div style={{fontFamily:'Amiri,serif',fontSize:24,color:'var(--gold)',lineHeight:1.9,letterSpacing:1}}>بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ</div>
          <div style={{fontSize:11,color:'var(--cream3)',letterSpacing:1.5,marginTop:2}}>Begin with the name of Allah</div>
        </div>
        {/* Feature carousel */}
        <div style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:14,padding:'16px',marginBottom:24,minHeight:100,position:'relative',overflow:'hidden'}}>
          {features.map((f,i)=>(
            <div key={i} style={{position:i===0?'relative':'absolute',top:0,left:0,right:0,padding:'16px',opacity:slide===i?1:0,transform:slide===i?'translateY(0)':'translateY(6px)',transition:'all 0.5s ease',pointerEvents:slide===i?'all':'none'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                <div style={{width:36,height:36,borderRadius:10,background:'var(--gold3)',border:'0.5px solid var(--border2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,flexShrink:0}}>{f.icon}</div>
                <div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,color:'var(--cream)',marginBottom:4}}>{f.title}</div>
                  <div style={{fontSize:12,color:'var(--cream3)',lineHeight:1.6}}>{f.desc}</div>
                </div>
              </div>
            </div>
          ))}
          <div style={{display:'flex',gap:4,justifyContent:'center',paddingTop:86}}>
            {features.map((_,i)=>(
              <div key={i} onClick={()=>setSlide(i)} style={{width:i===slide?16:5,height:5,borderRadius:3,background:i===slide?'var(--gold)':'var(--border2)',cursor:'pointer',transition:'all 0.3s'}}/>
            ))}
          </div>
        </div>
        {/* Name input */}
        <div style={{marginBottom:12}}>
          ...
          <input value={name} onChange={e=>{setName(e.target.value);setNameErr(false)}} onKeyDown={e=>{if(e.key==='Enter')handleBegin()}} placeholder="e.g. Sameed"
            style={{width:'100%',background:'var(--bg3)',border:`1px solid ${nameErr?'var(--red)':'var(--border2)'}`,borderRadius:12,padding:'14px 18px',color:'var(--cream)',fontSize:16,outline:'none',fontFamily:"'DM Sans',sans-serif"}}/>
          {nameErr&&<div style={{color:'var(--red)',fontSize:12,marginTop:6}}>Please enter your name to begin</div>}
        </div>
        <div style={{background:'rgba(200,164,90,0.06)',border:'0.5px solid var(--border)',borderRadius:10,padding:'10px 14px',marginBottom:16,display:'flex',gap:10,alignItems:'flex-start'}}>
          <span style={{fontSize:14,flexShrink:0}}>🔐</span>
          <div style={{fontSize:12,color:'var(--cream3)',lineHeight:1.7}}><span style={{color:'var(--gold)'}}>Unique accounts coming soon</span> — your progress will be fully secured.</div>
        </div>
        <button onClick={handleBegin} style={{width:'100%',background:'var(--gold)',border:'none',borderRadius:12,padding:'15px',color:'#08100A',fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,cursor:'pointer',letterSpacing:0.5,marginBottom:10}}>
          Begin Reading →
        </button>
        <button onClick={()=>onDone('Guest')} style={{width:'100%',background:'transparent',border:'0.5px solid var(--border)',borderRadius:12,padding:'13px',color:'var(--cream3)',fontFamily:"'Cormorant Garamond',serif",fontSize:14,cursor:'pointer',marginBottom:24}}>
          Continue as Guest
        </button>
        <div style={{textAlign:'center',fontSize:11,color:'var(--cream3)',lineHeight:1.9,opacity:0.7}}>
          All 114 Surahs · 5 Qaris · 8 Languages · Free Forever · No Ads
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{minHeight:'100vh',display:'flex',background:'var(--bg)',position:'relative',overflow:'hidden'}}>
      ...
      {/* Left panel */}
      <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',padding:'40px 48px',borderRight:'0.5px solid var(--border)',position:'relative'}}>
        ...
        <div style={{marginBottom:40,padding:'20px 24px',background:'var(--gold3)',border:'0.5px solid var(--border2)',borderRadius:16}}>
      ...
      {/* Right panel */}
      <div style={{width:420,flexShrink:0,display:'flex',flexDirection:'column',justifyContent:'center',padding:'40px 44px',position:'relative'}}>
        ...
        <div style={{background:'rgba(200,164,90,0.06)',border:'0.5px solid var(--border)',borderRadius:10,padding:'10px 14px',marginBottom:20,display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <div style={{fontSize:12,color:'var(--cream3)',lineHeight:1.7}}><span style={{color:'var(--gold)'}}>Unique accounts coming soon</span> — your identity and progress will be fully secured.</div>
        </div>
        <button onClick={handleBegin} style={{width:'100%',background:'var(--gold)',border:'none',borderRadius:12,padding:'15px',color:'#08100A',fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,cursor:'pointer',letterSpacing:0.5,marginBottom:10}}>
          Begin Reading →
        </button>
        ...
        <div style={{marginTop:14,fontSize:10,color:'var(--cream3)',textAlign:'center',opacity:0.5,lineHeight:1.9}}>
          All 114 Surahs · 5 Qaris · 8 Languages · Free Forever · No Ads
        </div>
      </div>
    </div>
...
function HomeView({userName,pts,completed,setView,openSurah,lastRead,liveEpisode,liveCount}){
  ...
  return(
    <div style={{paddingBottom:20}}>
      ...
      <div style={{position:'relative',overflow:'hidden',borderBottom:'0.5px solid var(--border)',maxWidth:'100%'}}>
